...
public class LicensepoolExpirationEvent {
	 ...
	 */
	public void checkForExpiration() throws JobExecutionException {
		...
		if (findExpiredLicensePool != null && findExpiredLicensePool.size() > 0) {
			for (String lplcId : findExpiredLicensePool) {
				...
				try {
					...
					logger.log(Level.ERROR, "Exception while sending Licensepool expiration message with ID :" + lplcId
							+ "  " + e.getStackTrace());
				}
			}
		}
