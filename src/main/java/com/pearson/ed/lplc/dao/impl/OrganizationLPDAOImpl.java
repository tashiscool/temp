...
public class OrganizationLPDAOImpl extends LPLCBaseDAOImpl implements OrganizationLPDAO, LPLCConstants {
	/**
	 * Gets license pools for the given organizationId, that are created at the organization or inherited from parent
	...
	/**
	 * Gets license pools for the given organizationIds, that are created at the organization or inherited from parent
	 * using the level attribute. level =0 means, license pools created for an organization.
	 * 
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<OrganizationLPMapping> listOrganizationMappingByOrganizationId(List<String> organizationIds, int level) {
		Criteria criteria = getSession().createCriteria(OrganizationLPMapping.class)
				.setFetchMode("licensepoolMapping", FetchMode.JOIN)
				.setFetchMode("licensepoolMapping.orderLineItems", FetchMode.JOIN)
