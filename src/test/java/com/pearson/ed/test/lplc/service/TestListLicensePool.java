...
public class TestListLicensePool extends BaseIntegrationTest {
	...
	public void testGetLicensePoolToSubscribeForSuccess() {
    	...
    	List<LicensePoolMapping> lpList =
    		licensepoolDAO.findOrganizationMappingToSubscribe(orgId,productId, currentDate, true);
    	assertEquals("Did not fetch both the qualifying licensepools", 2, lpList.size());
    	assertEquals("Did not fetch the latest license pool available", licensepoolId2,lpList.get(0).getLicensepoolId());
    	
    	// Check passing a date out of bound of the license pool's start and end dates
    	
    	lpList =
    		licensepoolDAO.findOrganizationMappingToSubscribe(orgId,productId, new Date("1/1/1800"), true);
    	
    	assertEquals("Returned results when the given date is out of bound of license pool's start and end dates", 0, lpList.size());
    	...
    	assertEquals("Did not fetch the latest license pool available", licensepoolId1,lpList.get(0).getLicensepoolId());
    ...
    public void testGetLicensePoolToSubscribeForLPWithDates() {
    	...
    	try{
        	...
        	// Service should raise a LicensePoolExpiredException as dates of License pool are out of bound from the current date
        	
        	try {
        		licensepoolService.getLicensePoolToSubscribeId(orgId, productId);
        	} catch(LicensePoolExpiredException lpExpiredException) {
        		assertTrue("LicensePoolExpiredException is not raised when all licensepools have denyNewSubscriptions set to " +
        	...
        	try {
        		...
        		assertTrue("LicensePoolUnavailableException is no license pools exist for the given orgId, productId", true);
        	}
