...
/**
 * A base class for all integration tests that load a spring application context and use dependency injection to
 * populate fields in the test class. Unlike AbstractDependencyInjectionSpringContextTests, this class can be used as a
 * base class for JUnit 4 tests.
 * 
 * RunWith is required to force what would otherwise look like a JUnit 3.x test to run with the JUnit 4 test runner.
 * 
 */
...
public abstract class BaseIntegrationTest extends AbstractTransactionalDataSourceSpringContextTests {
	...
	/**
	 * pass through to the junit 3 calls, which are not annotated. JUnit 4 only cares about annotations and won't run
	 * the old setUp and tearDown methods defined by the Spring classes. To get around this, add methods with Before and
	 * After annotations to call setUp and tearDown
	 * 
	 * @throws Exception
	 *             - Exception
	 */
	...
	/**
	 * Return the locations of the config files. A plain path will be treated as class path location. E.g.:
	 * "org/springframework/whatever/foo.xml". Note however that you may prefix path locations with standard Spring
	 * resource prefixes. Therefore, a config location path prefixed with "classpath:" with behave the same as a plain
	 * path, but a config location such as "file:/some/path/path/location/appContext.xml" will be treated as a
	 * filesystem location.
	 * 
