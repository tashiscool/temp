...
DECLARE x NUMBER;
BEGIN
SELECT COUNT(*) INTO x FROM user_indexes WHERE index_name='IDX_LICENSEPOOL_ORGANIZATION_2' AND table_name='LICENSEPOOL_ORGANIZATION' AND index_type='NORMAL';

IF x = 0 THEN
   EXECUTE IMMEDIATE 
   '
      CREATE INDEX "IDX_LICENSEPOOL_ORGANIZATION_2" ON "LICENSEPOOL_ORGANIZATION" ("ORGANIZATION_ID")
   ';
END IF;
